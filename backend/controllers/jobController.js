...
exports.getJobByType = async (req, req) => {
    try {
        ...
        if(jobs.length==0)
            return res.status(404).json({ message: "No jobs found in this type" });
        return res.json(jobs);
    } catch (error) {
        return res.status(500).json({ message: error.message });
    }
...
exports.getJobsByLocation = async (req, res) => {
    try {
        ...
        if(jobs.length==0)
            return res.status(404).json({ message: "No jobs found in this location" });
        return res.json(jobs);
    } catch (error) {
        return res.status(500).json({ message: error.message });
    }
...
exports.getJobsBySalaryRange = async (req, res) => {
    try {
        const jobs = await job.find({ salary: { $gte: req.params.min, $lte: req.params.max } });
        if(jobs.length==0)
